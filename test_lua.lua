...
for k, name in pairs(conditional_antiaims.conditions_names) do
	...
	conditional_antiaims.conditions[k].yaw_modifier_offset1338 = group:slider('Yaw jitter  »  Delay ticks' .. name_unique, 2, 20, 0, 0, 't')
	...
	if name ~= 'Fakelag' then
		...
		conditional_antiaims.conditions[k].defensive_yaw = group:combobox('Yaw\ndefensive_yaw' .. name_unique, {'Disabled', 'Sideways', 'Opposite', "Spin",  'Switch', 'Flick', 'Spin180', 'RandomYaw', 'RandomStaticYaw', 'Custom'})
...
experimental_defensive = group:checkbox('⚠  defensive')
...
conditional_antiaims.new_meta_antiaims = safecall('new_meta_antiaims', true, function(new_config)
    ...
    -- Используем один слайдер для задержки смены углов джиттера
    local jitter_delay = new_config.yaw_modifier_offset1338 or 3 -- Количество тиков для задержки

    -- Счетчик тиков
    conditional_antiaims.jitter_tick_counter = (conditional_antiaims.jitter_tick_counter or 0) + 1
    ...
    elseif new_config.yaw_modifier == 'Custom way' then
        if conditional_antiaims.current_jitter_side == nil then
            conditional_antiaims.current_jitter_side = true -- Начинаем с левого угла
            conditional_antiaims.jitter_tick_counter = 0
        end

        -- Проверяем, пора ли менять сторону
        if conditional_antiaims.jitter_tick_counter >= jitter_delay then
            conditional_antiaims.jitter_tick_counter = 0
            conditional_antiaims.current_jitter_side = not conditional_antiaims.current_jitter_side -- Меняем сторону
        end

        -- Устанавливаем нужный угол в зависимости от текущей стороны
        if conditional_antiaims.current_jitter_side then
            value = new_config.yaw_modifier_offset1 -- Левый угол
        else
            value = new_config.yaw_modifier_offset2 -- Правый угол
        end
...
for k, v in pairs(conditional_antiaims.conditions_names) do
	...
	conditional_antiaims.conditions[k].yaw_modifier_offset1338:depend({conditional_antiaims.conditions[k].yaw_modifier, 'Custom way'})
